...
| 🎨 Design system | 🟡 En cours | — | Tokens CSS du design system Claude Design (Bleu France #000091, Rouge Marianne #E1000F, Spectral/Hanken Grotesk/IBM Plex Mono) fusionnés dans `src/ministere_de_l_info/custom.css` + injectés via `_theme.py::inject_css()` sur les 5 pages. Sélecteurs Streamlit basés sur `reports/research-streamlit-css-selectors.md`. Bug de contraste sidebar (texte illisible) corrigé. Brainstorm UI/UX lancé (`reports/brainstorm-ui-ux-design-system.md`) : décisions actées — filtres sidebar (persistant) + inline (contextuel), bascule emojis → icônes vectorielles (librairie/intégration à cadrer). Reste à faire : uniformiser structure titre/filtres/contenu/table sur Élections, Législatif, Économie ; composant "metric row" réutilisable ; chantier icônes. |

**Dernier rapport** : `reports/brainstorm-ui-ux-design-system.md` — Design system CSS intégré (5 pages) + brainstorm UI/UX : diagnostic des incohérences entre pages et décisions de convention (filtres, icônes)
