...
| 🚀 Déploiement | ✅ Mac/OrbStack, testé | v0.5-economie-legislatif | Releases publiées : v0.4.1-deploy-init (packaging initial) puis v0.5-economie-legislatif (image + DB, 2026-06-28). Tags correctifs v0.4.2-actions-fix et v0.4.3-fix-spatial sans release GitHub. install.sh validé sur macOS 26.4.1. Image sur ghcr.io. DB sur GitHub Release. TODO : update.sh test réel, page Paramètres UI. |
| 📊 Économie | ✅ Phase E++ complète | v0.5-economie-legislatif | Sources : Filosofi + RP + logements sociaux + CNAF RSA + DREES APL + URSSAF + Eurostat. 7 indicateurs Phase E, Phase E+ : CNAF RSA + DREES APL + URSSAF (1.15M lignes 2006-2025). 943 déserts médicaux HdF détectés. Phase E++ : economie_contexte (Eurostat chômage BIT 1999-2025 + PIB/hab 2000-2024, HdF vs France). UI Streamlit : carte choroplèthe + évolution HdF + croisement économie×élections. 348 tests à la clôture E+ ; couverture 75.34 % mesurée avec base locale en Phase E (~60 % en CI sans base). ETL : load_economie.py (cache HdF 40Mo). UI Phase E+ : onglet Désindustrialisation (URSSAF 2006-2025), RSA et APL dans carte + évolution, déserts médicaux carte binaire. UI Phase E++ : Tab 2 section Eurostat. |
| 🏛️ Législatif | ✅ Phase F complète | v0.5-economie-legislatif | Périmètre national (4065 élus : 2120 AN législatures 12-17, 2002-présent ; 1945 Sénat, actifs et anciens du fichier ODSEN_GENERAL). Source Datan (scores activité AN, législatures 12-17) + Sénat CSV officiel (data.senat.fr). UI Streamlit : 4 onglets (composition politique, liste élus, activité parlementaire AN, évolution historique). Filtre national + HdF en UI. 3 tables (leg_elus, leg_activite, leg_blocs_override), 2 vues SQL, 10 fonctions requêtes. 381 tests (hors `slow`). Couverture : le rapport Phase F reprend 75.34 %, valeur identique au centième à celle de la Phase E, sans nouvelle mesure attestée ; ~60 % en CI sans base. Limitations connues : pas de scores Sénat (Datan = AN uniquement), pas de votes nominatifs (XML brut non parsé). |
...
| `docs/adr/` | 9 ADR (0007-0009 rédigés a posteriori) — décisions d'architecture non réversibles sans consensus |
