"""Page Élections — présidentielles Hauts-de-France (C3)."""
...
st.title("🗳️ Élections présidentielles")
st.caption("Résultats par bloc politique officiel du Ministère de l'Intérieur — Hauts-de-France")
...
# ── Référentiel blocs (immuable, chargé une fois) ────────────────────────────
...
with c1:
    annee: int = st.selectbox("Année", _ANNEES, index=len(_ANNEES) - 1)  # type: ignore[assignment]
with c2:
...
with c4:
    mode_carte: str = st.radio("Mode de carte", _MODES_CARTE)  # type: ignore[assignment]
    bloc_sel: str = bloc_codes[3]  # CENT par défaut si mode bloc dominant
...
if mode_evol == "Part des exprimés (%)":
    ...
    ev_plot = ev_t.join(totaux, on="annee").with_columns(
        (pl.col("voix_total").cast(pl.Float64) * 100.0 / pl.col("total").cast(pl.Float64)).alias(
            "pct"
        )
    )
...
# ── Zone 4 — tableau comparatif ──────────────────────────────────────────────

st.divider()
st.subheader("Détail par commune")

# Pivot scores long → wide (code_commune × bloc)
...
blocs_disponibles = sorted(table_df["bloc_dominant"].cast(pl.Utf8).drop_nulls().unique().to_list())
...
col_ordre = ["nom", "inscrits", "exprimes", "taux_participation_pct", "bloc_dominant"] + bloc_codes
